...
func TestCollectPublishWorkflow(t *testing.T) {
	...
	Convey("Given a started plugin control", t, func() {
		...
		Convey("create a workflow", func() {
			rp, err := core.NewRequestedPlugin(snap_collector_mock2_path, c.GetTempDir())
			So(err, ShouldBeNil)
			_, err = c.Load(rp)
			So(err, ShouldBeNil)
			rp2, err := core.NewRequestedPlugin(snap_publisher_file_path, c.GetTempDir())
			So(err, ShouldBeNil)
			_, err = c.Load(rp2)
			So(err, ShouldBeNil)
			rp3, err := core.NewRequestedPlugin(snap_processor_passthru_path, c.GetTempDir())
			So(err, ShouldBeNil)
			_, err = c.Load(rp3)
			So(err, ShouldBeNil)
			rp4, err := core.NewRequestedPlugin(snap_collector_mock1_path, c.GetTempDir())
			So(err, ShouldBeNil)
			_, err = c.Load(rp4)
			So(err, ShouldBeNil)
