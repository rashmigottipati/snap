...
func TestGRPCServerScheduler(t *testing.T) {
	...
	// processor -- passthru
	// publisher -- file
	mock, err := core.NewRequestedPlugin(fixtures.PluginPathMock1, GetDefaultConfig().TempDirPath)
	if err != nil {
		log.Fatal(err)
	}
	_, serr := c.Load(mock)
	...
	<-lpe.done
	passthru, err := core.NewRequestedPlugin(helper.PluginFilePath("snap-plugin-processor-passthru"), GetDefaultConfig().TempDirPath)
	if err != nil {
		log.Fatal(err)
	}
	...
	<-lpe.done
	filepub, err := core.NewRequestedPlugin(helper.PluginFilePath("snap-plugin-publisher-mock-file"), GetDefaultConfig().TempDirPath)
	if err != nil {
		log.Fatal(err)
	}
