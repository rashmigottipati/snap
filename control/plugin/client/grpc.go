...
func NewStreamCollectorGrpcClient(address string, timeout time.Duration, security GRPCSecurity) (PluginStreamCollectorClient, error) {
	ctx := context.WithValue(context.Background(), "MaxCollectDuration", "5s")
	ctx = context.WithValue(ctx, "MaxMetricsBuffer", 5)
...
func (g *grpcClient) StreamMetrics(mts []core.Metric) (chan []core.Metric, chan error, error) {
	...
	fmt.Printf("!!!!!!! calling stream - context MaxCollectDuration=%v\n", g.context.Value("MaxCollectDuration"))
	s, err := g.streamCollector.StreamMetrics(g.context)
	if err != nil {
		return nil, nil, err
	}
	err = s.Send(arg)
